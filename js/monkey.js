 ...
 * 
 *@author snow.he jun.zhu aaron.xu jaune.wang
 *
 *@Copyright  Modern Multimedia (ShenZhen) Company Limited
 *
 *@github https://github.com/snowHe2017/MONKEY
 *
 * 
 */
var MONKEY = { REVISION:'1.1.7'};
...
Object.assign(MONKEY,{
	...
	init:function(_parameters){
		...
		MONKEY.OSInfo.updataOsInfo();
		MONKEY.WorkInfo.initWorkInfo(parameters);

		MONKEY.pointer = new MONKEY.Pointer(parameters.canvas,parameters.render);
...
MONKEY.OSInfo = {
	...
	updataOsInfo:function(){
		...
		try{this.touchable=("ontouchstart" in document &&  !MONKEY.Common.isPC()) ? true : false;}catch(e){this.touchable=false;}
		try{this.isSurface = ("ontouchstart" in document && MONKEY.Common.isPC()) ? true : false}catch(e){}
		try{this.localStorage =	 'localStorage' in window && window['localStorage'] !== null; }catch(e){this.localStorage =	 false; }
		this.browser = MONKEY.Common.getBrowser();
		...
		MONKEY.Tween.countAbility = this.touchable ? 2 : 1;
...
MONKEY.PreloadStyle = {
	...
	staticStyle:function(ctx,_percent){
		var x = MONKEY.WorkInfo.pWidth/2,
			y = MONKEY.WorkInfo.pHeight/2;
		ctx.fillStyle = "#000";
...
MONKEY.Preload = {
	...
	loadingFile:function(_file,_showFunc,_callbacks,_waitTime){
		...
		for(var i = 0,len = _file.length; i < len; i ++){
			...
			if(!nowFile["type"]){
				...
				var ext = MONKEY.Common.getExtension(path);
				switch(ext){
					case "txt":
						nowFile.type = "text";
						nowFile.path = path;
			...
			var name = MONKEY.Math.generateUUID();
			if(nowFile["name"]){
				if(this.fileList.indexOf(nowFile.name) < 0){
					name = nowFile.name;
				}else{
					console.error("MONKEY.Preload file's name repeat:" + nowFile.name);
					return;
				}
			} 
			switch(nowFile.type){
				...
				case "js":
					...
					break;
				case "audio":
					MONKEY.Audio.createAudio([nowFile]);
					break;
				case "img":
					var ele = new Image();
					ele.onload = this.fileOnload;
		...
		if(typeof _showFunc == "function"){
			var n = MONKEY.PreloadStyle.cacheStyle = _showFunc;
	...
	updateProgress:function(ctx){
		...
		this.progress = (nowf / maxf * 100).toFixed(0);
		if( nowf >= maxf || userTime >= this.waitTime ){
			this.progress = 100;
			this.callback(this.fileData);
		}
		MONKEY.PreloadStyle[this.showType].call(this,ctx,this.progress);
	},
	addShowType:function(name,_func){
		if(!MONKEY.PreloadStyle[name]){
			MONKEY.PreloadStyle[name] = _func;
		}else{
			console.error("MONKEY.Preload.addShowType: name repeat.");
		}
	},
...
MONKEY.Loader = {
	...
	jsonp:function(_options){
		...
		var url = options.url !== undefined ? options.url :"",
			...
			callbacks = ("MONKEY_JSONP_"+Math.random()).replace(".","");
...
MONKEY.Matrix2.prototype = {
	...
	getElement:function(){
		return MONKEY.Common.arrayCopy(this.elements);
	},
	setFixed:function(n){
		if(!isNaN(n)){
			if(n >= 0){
...
MONKEY.Common = {
	...
	traceDebug:function(){
		...
		if(!output){
			...
			ele.appendChild(btn);
			MONKEY.MouseEvent.addEvent(scaleBtn, "mousedown",function(e){
			...
			MONKEY.MouseEvent.addEvent(closeBtn,"mousedown",function(e){
				e.preventDefault && e.preventDefault();
				ele.style.display = "none";
				e.stopPropagation && e.stopPropagation();
			},true);
			MONKEY.MouseEvent.addEvent(delBtn,"mousedown",function(e){
				e.preventDefault && e.preventDefault();
				output.value = "";
				e.stopPropagation && e.stopPropagation();
			},true);
			...
			var mouseData = [0,0];
			MONKEY.MouseEvent.addEvent(ele,"mousedown",function(e){
				e.preventDefault && e.preventDefault();
				var emouse = MONKEY.Common.getMouse(e);
				mouseData = [emouse.x,emouse.y];
				MONKEY.MouseEvent.addEvent(document.body,"mousemove",eleMove);
				MONKEY.MouseEvent.addEvent(document.body,"mouseup",eleUp);
			},false);

			function eleMove(e){
				e.preventDefault && e.preventDefault();
				var nMouse = MONKEY.Common.getMouse(e);
			...
			function eleUp(e){
				MONKEY.MouseEvent.removeEvent(document.body,"mousemove",eleMove);
				MONKEY.MouseEvent.removeEvent(document.body,"mouseup",eleUp);
			}
			output = document.createElement("textarea");
			output.id = "monkeyTraceOutput";
			output.style.width = maxWidth * 0.4 + "px";
			output.style.height = "200px";
			MONKEY.MouseEvent.addEvent(output,"mousedown",function(e){
				e.stopPropagation && e.stopPropagation();
			})
	...
	PointInOrOnPoly:function(pt,poly){
		 for(var c = false,i = -1,l = poly.length, j = l - 1; ++ i < l; j = i){
			if(MONKEY.Math.getDistancePointToSegment(poly[j][0],poly[j][1],poly[i][0],poly[i][1],pt.x,pt.y) == 0){
				return true;
			}else{
				((poly[i][1] <= pt.y && pt.y < poly[j][1]) || (poly[j][1] <= pt.y && pt.y < poly[i][1]))
			&& (pt.x > (poly[j][0] - poly[i][0]) * (pt.y - poly[i][1]) / (poly[j][1] - poly[i][1]) + poly[i][0])
	...
	hitTestPolygon:function (p1, p2) {
		...
		var tp = new MONKEY.Position();
		for (j = 0; j < list.length; j++) {
			listA = list[j][0], normals = list[j][1];
			for (i = 0, l = listA.length; i < l; i++) {
				list[j][2].push(new MONKEY.Position(listA[i][0], listA[i][1]));
				if (i < l - 1) {
					normals.push((new MONKEY.Position(listA[i + 1][0] - listA[i][0], listA[i + 1][1] - listA[i][1])).cloneReverseY());
				}
			}
			normals.push((new MONKEY.Position(listA[0][0] - listA[l - 1][0], listA[0][1] - listA[l - 1][1])).cloneReverseY());
		}
		for (j = 0; j < list.length; j++) {
			normals = list[j][1];
	...
	hitTestPolygonArc:function (vs, arc) {
		if (MONKEY.Common.hitPolygon(vs, arc[0], arc[1])) {
			return true;
		}
		var tp = new MONKEY.Position();
		...
		for (i = 0, l = vs.length; i < l; i++) {
			j = i < l - 1 ? i + 1 : 0;
			p1 = vs[i], p2 = vs[j];
			v1 = new MONKEY.Position(arc[0] - p1[0], arc[1] - p1[1]), v2 = new MONKEY.Position(p2[0] - p1[0], p2[1] - p1[1]);
			l2 = v2.cloneAver();
			inn = tp.dot(v1, l2);
			if (inn <= 0) {
...
MONKEY.Sint = {
	...
	add:function(_name,_func,_timer){
		if(typeof _func != "function" || typeof _name != "string" || isNaN(_timer)){
			console.warn("MONKEY.Sint.add:arguments error.");
			return;
		}
		var obj = {
			name:_name,
	...
	changeStatus:function(_name,_status){
...
MONKEY.Math = {
	 ...
	 ClockwiseSortPoints:function(_polygonArr){
		...
		for(var i = 0; i < _polygonArr.length - 1; i++){
			for(var j = 0; j < _polygonArr.length - i - 1; j ++){
				if(MONKEY.Math.pointCmp(_polygonArr[j], _polygonArr[j+1], center)){
					var tempPoint = [];
	...
	getPolygonClip:function(arr1,arr2){
		...
		for(var i = arr1.length - 1; i >= 0; i --){
			pt.x = arr1[i][0];
			pt.y = arr1[i][1];
			if(MONKEY.Common.PointInOrOnPoly(pt,arr2)){
		...
		for(var j = arr2.length - 1; j >= 0; j --){
			pt.x = arr2[j][0];
			pt.y = arr2[j][1];
			if(MONKEY.Common.PointInOrOnPoly(pt,arr1)){
		...
		for(var m = arr1.length- 1; m >= 0; m --){
			...
			for(var n = arr2.length - 1; n >= 0; n --){
				...
				ep2.x = arr2[enext][0];
				ep2.y = arr2[enext][1];
				var res = MONKEY.Math.segmentsIntr(sp1,sp2,ep1,ep2);
				if(res){
		...
		trigger = MONKEY.Math.ClockwiseSortPoints(nTrigger);
		return trigger;
	},
	...
	getDistancePointToSegment:function(x1,y1,x2,y2,x,y){
		...
		var ang = MONKEY.Math.getThreePointAngle(x1,y1,x2,y2,x,y);
		if(ang%360 == 0){
			return 0;
		}
		return MONKEY.Math.getDistancePointToLine(x1,y1,x2,y2,x,y);
	},
	...
	isParallel:function(dot1,dot2,dot3,dot4){
		var status = false;
		var angle1 = MONKEY.Math.setCallAngle(dot1.x,dot1.y,dot2.x,dot2.y),
			angle2 = MONKEY.Math.setCallAngle(dot3.x,dot3.y,dot4.x,dot4.y);
		if(angle1 == angle2){
			var angle3 = MONKEY.Math.setCallAngle(dot1.x,dot1.y,dot3.x,dot3.y),
				angle4 = MONKEY.Math.setCallAngle(dot2.x,dot2.y,dot4.x,dot4.y);
			if(angle3 != angle4){
				status = true;	
			}
		}
	...
	getOverflow:function(ele){
		return ele.parent == null?ele.overflow : (ele.overflow == "inherit" ? MONKEY.Math.getOverflow(ele.parent) : ele.overflow);
	},
...
MONKEY.Tween = {
	...
	canvasAni:function(){
		...
		if(arguments.length < 2){
			console.warn("MONKEY.Tween.canvasAni is a function must more than 2 arguments.");
			return;
		}
 ...
 MONKEY.Renderer = function(parameters){
	...
	this.sceneElm = [];
	this.sint;
	MONKEY.init({width:width_default,height:height_default,canvas:this.canvas,render:this});
 ...
 MONKEY.Renderer.prototype = {
	...
	begin:function(){
		...
		if(MONKEY.OSInfo.requestAnimationFrameAble){
	...
	render:function(t){
		...
		if(parent.Preload.progress < 100){
			...
			MONKEY.Common.rankScene(this.sceneElm);
	...
	renderDraw:function(_object){
		...
		MONKEY.Common.rankScene(_object.children);
	},
	renderDrawImageObject:function(_object,showImage){
		...
		var overflow = _object.overflow == "hidden" ? "hidden":(_object.overflow == "inherit" ? MONKEY.Math.getOverflow(_object) : "visible");
		if(overflow == "hidden"){
			bufferCtx.beginPath();
	...
	renderDrawSameGrapAnimation:function(_animation){
		if(_animation.visible){
			...
			var overflow = _animation.overflow == "hidden" ? "hidden":(_animation.overflow == "inherit" ? MONKEY.Math.getOverflow(_animation) : "visible");
			if(overflow == "hidden"){
					bufferCtx.beginPath();
					bufferCtx.rect(0,0,w,h);
					bufferCtx.clip();
	...
	renderDrawText:function(_text){
		if(_text.visible){
			...
			if(_text.textStr != null){
				...
				var overflow = _text.overflow == "hidden" ? "hidden":(_text.overflow == "inherit" ? MONKEY.Math.getOverflow(_text) : "visible");
				if(overflow == "hidden"){
					bufferCtx.beginPath();
...
MONKEY.DeviceListener = {
	...
	addListener:function(eventType,fnHandler){
		if(typeof fnHandler !== "function"){
			console.error("MONKEY.EventListener.addListener parameter fnHandler must a function.");
			return;
		}		
	...
	handleOrientation:function(event){
		...
		var self = MONKEY.DeviceListener;
...
MONKEY.Signal = function(){
	this.active = true;
	this.disableList = [];
};
MONKEY.Signal.prototype = {
	constructor:MONKEY.Signal,
	...
	add:function(_type,listener){
		if(_type === undefined){
			throw new Error("MONKEY.Signal.add:_type is undefined.");
			return;
		}
		if(typeof listener !== "function"){
			throw new Error("MONKEY.Signal.add:listener is not function. ");
			return;
		} 
	...
	remove:function(_type,listener){
		if(_type === undefined){
			throw new Error("MONKEY.Signal.remove:_type is undefined.");
			return;
		}
		if(typeof listener !== "function"){
	...
	publish:function(_type){
		if(_type === undefined){
			throw new Error("MONKEY.Signal.publish:_type is undefined.");
			return;
		}
...
MONKEY.Pointer = function(_target,_render){
	var self		= this;
	...
	this.mouseListener	= {}; 
	this.matrix2 = new MONKEY.Matrix2();
	this.matrix3 = new MONKEY.Matrix3();
	this.eventMatrixStack = [];
	this.eventMouseList = [];
	this.eventListHistory = [];
	this.eventRotateMatrix = new MONKEY.Matrix3();
...
MONKEY.Pointer.prototype = {
	...
	updateOffset:function(){
		MONKEY.WorkInfo.canvasLeft = MONKEY.Math.getPageX(this.target) + MONKEY.WorkInfo.pWidth/2*(1 - MONKEY.WorkInfo.scale);
		MONKEY.WorkInfo.canvasTop  = MONKEY.Math.getPageY(this.target) + MONKEY.WorkInfo.pHeight/2*(1 - MONKEY.WorkInfo.scale);
	},
	updataMouse:function(e){
		var e = e || window.event;
		this.updateOffset();
		var workInfo = MONKEY.WorkInfo;
	...
	getEventTarget:function(_scene){
		...
		if(_scene.pathStatus){
			...
			var point = new MONKEY.Position(this.pX,this.pY);
			...
			if(pTrigger.length == 1 && pTrigger[0] == 0){
				...
				if(pTrigger.length == 0){
					...
					nowTrigger = MONKEY.Math.getPolygonClip(pTrigger,nowTrigger);
				}
			_scene.cacheTrigger = nowTrigger;
			status = MONKEY.Common.PointInPoly(point,nowTrigger);	
		}
	...
	getParentTrigger:function(ele){
		if(!ele || ele.parent == null){
			...
			if(ofl == "inherit"){
				ofl = MONKEY.Math.getOverflow(ele.parent);
			}
			if(ofl == "hidden"){
				return ele.cacheTrigger;
			}else{
				return MONKEY.pointer.getParentTrigger(ele.parent);
			}
		}
	},
	...
	publishEvent:function(list,type,mEvent,mouseEvent,_useCursor){
		...
		this.propagation = true;
		for(var i = j = 0; j >= 0 ;	 i ++ < len - 1 ? j = i : j --){
			var n = len - 1 - i;
			if(this.propagation){
...
MONKEY.AudioClass.prototype = {
	...
	getAudioDuration:function(_name){
		var name = _name,self = MONKEY.Audio;
...
MONKEY.Animal = function(_parameters){
	...
	if(!!parameters.background && typeof(parameters.background) == "string"){
		var pfile = MONKEY.Preload.getFileByUrl(parameters.background);
		if(pfile){
...
MONKEY.Animal.prototype = Object.assign(MONKEY.EventListener.prototype,{
	...
	changeAlpha:function(_alpha){
		var alpha = MONKEY.Math.clamp(_alpha,0,1);
		this.alpha = alpha;
	},
...
MONKEY.Scene.prototype = Object.assign(Object.create(MONKEY.Animal.prototype),{
...
MONKEY.Button.prototype = Object.assign(Object.create(MONKEY.Animal.prototype),{
...
MONKEY.IntervalAnimation.prototype = Object.assign(Object.create(MONKEY.Animal.prototype),{
...
MONKEY.SameGraphAnimation.prototype = Object.assign(Object.create(MONKEY.Animal.prototype),{
...
MONKEY.Text.prototype = Object.assign(Object.create(MONKEY.Animal.prototype),{
	...
	showOrHideShadow:function(_boo){
		if(_boo !== undefined){
			...
			this.shadow = this.shadown ? false : true;
		}
	},
...
MONKEY.Input = function(_parameters){
	var parameters = _parameters || {};
	...
	this.sMouseOutFunc = function(data){
		...
		MONKEY.pointer.removeMouseListener("mousedown",self.sMouseOutFunc);
		self.onBlur();
	};
...
MONKEY.Input.prototype = Object.assign(Object.create(MONKEY.Text.prototype),MONKEY.EventListener.prototype,{
	constructor:MONKEY.Input,
	sMouseDownFunc:function(){
		...
		var inputLeft = MONKEY.Math.getPositionX(this),
			inputTop = MONKEY.Math.getPositionY(this);
		this.inputBox.style.left = inputLeft + "px";
		this.inputBox.style.top = inputTop + "px";
		MONKEY.WorkInfo.canvas.parentNode.appendChild(this.inputBox);
		this.inputOpenStatus = true;
		this.inputBox.focus();
		setTimeout(function(){MONKEY.pointer.addMouseListener("mousedown",self.sMouseOutFunc);},0);
	},
	...
	draw:function(canvas,ctx,renderer){
		...
		if(this.textStr != ""){
			...
			if(textWidth > MONKEY.Math.getTrueLength(this.textStr)+2){
...
MONKEY.Select = function(_parameters){
	var self = this;
	var parameters = _parameters || {};
	MONKEY.Text.call(this,parameters);
	this.type = "Select";
	...
	this.dom = new MONKEY.Animal({name:this.name,x:this.position.x,y:this.position.y});

	this.selectLayer = new MONKEY.Animal({name:"selectLayer",listenerStatus:true,width:this.special.width,height:this.special.height,overflow:"hidden"});
	this.selectBg = new MONKEY.RectangleRoundGraphics({fillStyle:this.background,fillStatus:true,strokeStatus:true,strokeStyle:this.borderColor,radius:this.radius,width:this.special.width,height:this.special.height});
	...
	this.selectText = new MONKEY.Text(txtPara);
	this.selectBtn = new MONKEY.RegularPolygonGraphics({centre:[0,0],edges:3,rotate:60,fillStyle:"#000000",fillStatus:true,
	...
	//option
	this.optionDom = new MONKEY.Animal({name:"testDom",width:this.special.width,height:this.options.length * (this.fontSize + this.optionAdd)});

	this.optionBg = new MONKEY.RectangleRoundGraphics({fillStyle:this.background,fillStatus:true,ghost:true,strokeStatus:true,strokeStyle:this.borderColor,radius:this.radius,width:this.special.width,height:this.options.length * (this.fontSize + this.optionAdd)});
	this.optionList = new MONKEY.Animal({width:this.special.width,height:this.options.length * (this.fontSize + this.optionAdd),zIndex:10});
	...
	this.optionLayer = new MONKEY.ScrollTool({y:this.special.height,mc:this.optionDom,width:this.special.width,height:this.size * (this.fontSize + this.optionAdd),fontSize:this.fontSize,scrollBtnHeight:scrollBtnHeight});
	this.updataOption();
...
MONKEY.Select.prototype = Object.assign(Object.create(MONKEY.Animal.prototype),{
	constructor:MONKEY.Select,
	updataOption:function(){
		...
		for(var i = 0; i < this.options.length; i ++){
			...
			var option = new MONKEY.Animal({name:"option_" + i,y:y,width:this.special.width,height:this.fontSize + 3,listenerStatus:true});
			var optionHover = new MONKEY.RectangleRoundGraphics({name:"hover",y:hoverY,ghost:true,width:this.special.width,height:this.fontSize,fillStatus:true,strokeStatus:false,fillStyle:this.focusBackground,radius:2,visible:false});
			var optionHtml =  new MONKEY.Text({name:"txt",ghost:true,txt:this.options[i], x:5, y:0, lineWidth:this.special.height/2 + this.fontSize/4, width:this.special.width,fillStyle:"#000000", font:this.fontSize + "px Arial",fontSize:this.fontSize,textAlign:"left",indent:true,zIndex:10,textBaseline:"top"});
	...
	changeSize:function(num){
		var size = MONKEY.Math.clamp(size,1,this.options.length);
		this.size = size;
		if(this.size >= this.options.length){
			this.optionLayer.hideBtn();
		}else{
	...
	setSelectedIndex:function(index){
		var index = MONKEY.Math.clamp(index,0,this.options.length - 1);
		this.check = index;
	},
...
MONKEY.ScrollTool = function(_parameters){
	var self = this;
	var parameters = _parameters || {};
	MONKEY.Animal.call(this,parameters);
	...
	this.dom = new MONKEY.Animal({name:this.name,x:this.position.x,y:this.position.y,width:this.direction ? this.special.width : this.special.width + this.scrollWidth,height:this.special.height,overflow:"hidden"});

	this.scrollLayer = new MONKEY.RectangleRoundGraphics({x:this.direction ? this.special.width - this.scrollWidth : this.special.width,y:0,fillStyle:"#FFFFFF",fillStatus:true,zIndex:this.mc.zIndex + 10,strokeStatus:true,strokeStyle:"#999999",radius:this.radius,width:this.scrollWidth,height:this.special.height});
	this.upBtn = new MONKEY.RectangleRoundGraphics({fillStyle:"#FFFFFF",fillStatus:true,listenerStatus:true,strokeStatus:true,strokeStyle:"#999999",radius:this.radius,width:this.scrollWidth,height:this.scrollWidth});
	this.upBtn.add(new MONKEY.RegularPolygonGraphics({edges:3,rotate:0,fillStatus:true,radius:this.radius,x:this.scrollWidth/2,y:this.scrollWidth/2}));
	this.downBtn = new MONKEY.RectangleRoundGraphics({y:this.special.height - this.scrollWidth,fillStyle:"#FFFFFF",fillStatus:true,listenerStatus:true,strokeStatus:true,strokeStyle:"#999999",radius:this.radius,width:this.scrollWidth,height:this.scrollWidth});
	this.downBtn.add(new MONKEY.RegularPolygonGraphics({edges:3,rotate:60,fillStatus:true,radius:this.radius,x:this.scrollWidth/2,y:this.scrollWidth/2}));
	this.scrollBtn = new MONKEY.RectangleRoundGraphics({y:this.scrollWidth,fillStyle:"#C1C1C1",fillStatus:true,listenerStatus:true,strokeStatus:true,strokeStyle:"#999999",radius:this.radius,width:this.scrollWidth,height:this.scrollBtnHeight});
	...
	MONKEY.pointer.addMouseListener("mouseup",function(e,data){
		self.scrollChange = 0;
		self.scrolling = false;
	});
	MONKEY.pointer.addMouseListener("mousemove",function(e,data){
...
MONKEY.ScrollTool.prototype = Object.assign(Object.create(MONKEY.Animal.prototype),{
	constructor:MONKEY.ScrollTool,
	hideBtn:function(){
	...
	changeScrollBtn:function(num){
		...
		by = by + num;
		by = MONKEY.Math.clamp(by,this.minBtn,this.maxBtn);
		...
		py = MONKEY.Math.clamp(py,this.dom.special.height - this.mc.special.height,0);
	...
	changeScrollMC:function(num){
		...
		py = MONKEY.Math.clamp(py,minY,maxY);
...
MONKEY.TextArea = function(_parameters){
	var parameters = _parameters || {};
	MONKEY.Text.call(this,parameters);
	this.type = "TextArea";
...
MONKEY.TextArea.prototype = Object.assign(Object.create(MONKEY.Text.prototype),{
	constructor:MONKEY.TextArea,
	sMouseDownFunc:function(){
		var self = this;
		if(!this.useable) return;
		this.inputBox.type = this.inputType;
		...
		this.inputOpenStatus = true;
		this.inputBox.focus();
		setTimeout(function(){MONKEY.pointer.addMouseListener("mousedown",sMouseOutFunc);},0);
		function sMouseOutFunc(data){
			self.inputOpenStatus = false;
			if(self.boxToInput) self.boxToInputFunc();
			self.inputBox.style.display = "none";
			MONKEY.pointer.removeMouseListener("mousedown",sMouseOutFunc);
			self.onBlur();
		}
	},
	publishEvent:function(type,_useCapture){
	...
	blur:function(){
		...
		MONKEY.pointer.mouseListenerUI("mousedown",data);
		this.onBlur();
	},
	onBlur:function(){
		this.publishEvent("onblur",true);
	...
	countTxtArr:function(){
		...
		for(var i = 0,len = this.textStr.length; i < len; i ++){
			if(MONKEY.Math.getTrueLength(this.textStr[i]) > this.cols){
				var nArr = this.textStr[i].split(" ");
				var nCols = 0,nStr = "";
				for(var j = 0,len2 = nArr.length; j < len2; j ++){
					nStr += nArr[j];
					nCols += MONKEY.Math.getTrueLength(nArr[j]);
					if(nCols >= this.cols || (j != nArr.length - 1 && nCols + MONKEY.Math.getTrueLength(nArr[j + 1]) >=	 this.cols) || j == nArr.length - 1){
						showArr.push(nStr);
						nStr = "";
						nCols = 0;
					}
...
MONKEY.TimeText.prototype = Object.assign(Object.create(MONKEY.Text.prototype),{
...
MONKEY.HtmlText = function(_parameters){
	var parameters = _parameters || {};
	MONKEY.Text.call(this,parameters);
	this.type = "HtmlText";
...
MONKEY.HtmlText.prototype = Object.assign(Object.create(MONKEY.Text.prototype),{
	constructor:MONKEY.HtmlText,
	setText:function(_str){
...
MONKEY.Graphics.prototype = Object.assign(Object.create(MONKEY.Animal.prototype),{
...
MONKEY.GridHelper = function(_parameters){
	var parameters = _parameters || {};
	MONKEY.Animal.call(this,parameters);
...
MONKEY.GridHelper.prototype = Object.assign(Object.create(MONKEY.Animal.prototype),{
	constructor:MONKEY.GridHelper,
...
MONKEY.ArcGraphics = function(_parameters){
	...
	this.startAngle = MONKEY.Math.degToRad(this.start);

	this.end = parameters.end !== undefined ? parameters.end : 360;
	this.endAngle = MONKEY.Math.degToRad(this.end);
...
MONKEY.LineGraphics = function(_parameters){
	...
	this.rotatePoint = new MONKEY.Position(this.startPoint[0],this.startPoint[1]);
...
MONKEY.BezierGraphics = function(_parameters){
	var parameters = _parameters || {};
	MONKEY.Graphics.call(this,parameters);
...
MONKEY.BezierGraphics.prototype = Object.assign(Object.create(MONKEY.Graphics.prototype),{
	contructor:MONKEY.BezierGraphics,
	setStartPoint:function(_startPoint){
		this.startPoint = _startPoint;
	},
	getStartPoint:function(){
...
MONKEY.RectangleRoundGraphics = function(_parameters){
	var parameters = _parameters || {};
...
MONKEY.RectangleRoundGraphics.prototype = Object.assign(Object.create(MONKEY.Graphics.prototype),{
	constructor:MONKEY.RectangleRoundGraphics,
	setRadius:function(_radius){
		if(!isNaN(_radius))
			this.radius = _radius;
	},
...
MONKEY.EllipseGraphics = function(_parameters){
	var parameters = _parameters || {};
	MONKEY.Graphics.call(this,parameters);
	this.type = "EllipseGraphics";
}
MONKEY.EllipseGraphics.prototype = Object.assign(Object.create(MONKEY.Graphics.prototype),{
	constructor:MONKEY.EllipseGraphics,
	getCentre:function(){
		var width = !!this.special.width ? this.special.width : 100,
			height = !!this.special.height ? this.special.height : 100;
